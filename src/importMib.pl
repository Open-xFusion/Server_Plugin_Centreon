...
my $tmpMibFile="/tmp/tmpxFusionMib";
my $tmpConfFile="/tmp/tmpxFusionConf";
...
#if version of val_1 is larger than val_2 , return 1
sub compareVersion{
    my ($input_1,$input_2) = @_;
	my @arr_1 = @$input_1;
	my @arr_2 = @$input_2;
    if ($#arr_1 > $#arr_2){
	    for ($a = 0; $a < $#arr_1 - $#arr_2; $a++){
		    push @arr_2, 0;
		}
	}elsif ($#arr_1 < $#arr_2){
	    for ($a = 0; $a < $#arr_2 - $#arr_1; $a++){
		    push @arr_1, 0;
		}
	}
	for($cur = 0; $cur < $#arr_1; $cur ++){
	    if ($arr_1[$cur] > $arr_2[$cur]){
		    return 1;
		}elsif ($arr_1[$cur] < $arr_2[$cur]){
		    return -1;
		}
	}
	return 0;
...
sub main{   
    ...
    if ($result[0] =~ /(.*)\s*version ([\d]+.[\d]+.[\d]+) .*/ ){
        @arr_1 = split(/\./, $2);
        @arr_2 = split(/\./, "2.8.22");
        if (compareVersion(\@arr_1, \@arr_2) >= 0)
        {
